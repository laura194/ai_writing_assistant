...
const CommunityPage = () => {
    ...
    const handleDelete = async (id: string) => {
        // Das entsprechende Projekt finden
        const projectToDelete = projects.find((p) => p._id === id);

        // Prüfen, ob der aktuelle Benutzer der Ersteller ist
        const isOwner = projectToDelete?.username === user?.username;

        if (!isOwner) {
            toast.error("You can only delete projects you created yourself.", {
                duration: 5000,
                icon: "🔒",
                style: {
                    background: "#2a1b1e",
                    ...
                    >
                        ...
                        >
                            <div className="w-full max-w-3xl bg-[#e9e5f8] dark:bg-[#1e1538] rounded-3xl px-10 py-10 shadow-[0_0_40px_rgba(251,146,60,0.2)]">
                                ...
                                >
                                    This is an overview of all uploaded projects by the community. You can
                                    view, comment and upvote projects.
                                </p>
                                {loading ? (
