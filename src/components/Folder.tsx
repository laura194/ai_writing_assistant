...
import { ChevronUpIcon, ChevronDownIcon } from "@heroicons/react/24/solid"; // Importiere die Pfeil-Icons
interface FolderProps {
  ...
  onRenameOrIconUpdate: (updatedNode: Node) => void;
  isVisible?: boolean;
}
function Folder({
  node,
  ...
  const [isCollapsed, setIsCollapsed] = useState(false); // Zustandsvariable für das Ein-/Ausklappen der Kinder
  // Lokale Zustände
  ...
  const handleCollapseToggle = () => {
    setIsCollapsed((prev) => !prev); // Toggle den Zustand für das Ein-/Ausklappen
  };
  return (
    <li
    ...
    >
      <div className="flex items-center gap-2">
        {/* Button zum Ein-/Ausklappen der Kinder (Pfeile) */}
      ...
      {/* Kinder nur anzeigen, wenn sie nicht eingeklappt sind */}
